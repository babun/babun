...
function check_only_one_running {
    ...
    if mkdir /var/lock/$proc_name 2>/dev/null ; then
        ...
        echo "You can run only one instance of $proc_name. Close all other instances of $proc_name or clear the /var/lock/ directory if you are sure there are no other instances currently running."
        exit 1;
    fi
}
